...
#  GIT, Boilermaker can be a powerful allie.
#
# === Radmind
...
require 'boilermaker/boilermakerlib'
require 'boilermaker/pkgproj'
require 'boilermaker/quickpkg'
require 'boilermaker/radmind'
require 'date'
require 'erb'
require 'etc'
require 'optparse'
...
BM_VERSION = '0.4.9'
RADMINDTOOLSPATH = '/usr/local/bin/'
RADMINDTOOLS = ['fsdiff', 'ktcheck', 'lapply', 'lcksum', 'lcreate', 'lfdiff', 'lmerge', 'lsort', 'twhich']
RADMINDPATH = '/var/radmind/client/'
PKGMAKER = '/Developer/usr/bin/packagemaker'
HDIUTIL = '/usr/bin/hdiutil'
TEMPLATES = '/Library/Ruby/Site/1.8/boilermaker/templates'
RESOURCES = '/Library/Ruby/Site/1.8/boilermaker/resources'
SCRIPTS = '/Library/Ruby/Site/1.8/boilermaker/scripts'
TAG = "Package created using Boilermaker, version " + BM_VERSION
TMP = '/tmp'
APPS = '/Applications'
INFO = '/Contents/Info.plist'
TOKENDEFS = '/Contents/Resources/TokenDefinitions.plist'

include OSX

# Aliasing a RubyCocoa class method for convenience
class OSX::NSDictionary
  objc_alias_class_method('load:', 'dictionaryWithContentsOfFile:')
end

# Aliasing a RubyCocoa instance method for convenience
class OSX::NSCFDictionary
  objc_alias_method('store:', 'writeToFile:atomically:')
end

# Aliasing a RubyCocoa class for convenience
class Plist < OSX::NSDictionary
end

app = Boilermaker.new(ARGV)
app.run
